...
class DeribitOracle:
        ...
        while self._running:
            try:
                async with websockets.connect(uri) as ws:
                    log.info("Binance @bookTicker WS Connected")
                    async for msg in ws:
                        data = json.loads(msg)
    ...
    async def _deribit_chain_ws(self):
        """Websocket connection polling Option Chain every 2.5s"""
...
class PolymarketFeed:
    ...
    async def _ws_poly(self):
        while self._running:
            ...
            # Polymarket WS actually allows sending all IDs at once! Splitting causes them to override previous subscriptions.
            tids = []
    ...
    def _handle_poly(self, d):
        ...
        if et == "price_change":
            # The asset_id might be inside the changes array instead of top level
            for c in (d.get("changes") or d.get("price_changes") or []):
                ...
                # Fetch existing book or skip if not seeded
